...
public class HideObjects2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (_LastTransforms.Count > 0)
        {
            foreach (Transform t in _LastTransforms.Keys)
            {
                ...
                //_ObjectRenderer.material = _LastTransforms[t];
        ...
        if (hits.Length > 0)
        {
            foreach (RaycastHit hit in hits)
            {
                if (hit.collider.gameObject.transform != WatchTarget && hit.collider.transform.root != WatchTarget)
                {
                    ...
                    _LastTransforms.Add(hit.collider.gameObject.transform, mat);
                    mat.shader = HiderShader;
